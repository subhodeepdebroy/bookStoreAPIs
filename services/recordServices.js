        ...
        if (userData.isAdmin) {
          ...
          if (data === 0) {
            ...
            if (recordObj.length === 0) {
              //res.status(200).json(response(true, null, `No Money Spent in last ${params.days} Days `));
              return [null, `No Money Spent in last ${params.days} Days `];
            } else {
              //res.status(200).json(response(true, recordObj[0].expence, `This Much Money Spent in last ${params.days} Days`))
